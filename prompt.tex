...
\title{Parametric Polymorphism in C++, Java, and C\#:\\Generics vs. Templates and Metaprogramming Power\\[0.5em]\large Prompt Log (Reorganized by Task)}
...
edundant technical information. This redundancy was intentional; it served as the critical first step in the verification 
process, allowing for the cross-referencing of the model'overlapping sections and rs responses 
to ensure internal consistency before synthesizing the final master prompt.
...
\noindent\textbf{Writing Style Constraint}
\begin{itemize}
    \item Formal, objective, precise scientific style comparable to ACM/IEEE publications.
    \item Use accurate terminology: \emph{monomorphization}, \emph{name mangling}, \emph{RTTI}, \emph{type erasure}, \emph{bridge method}, \emph{reification}, \emph{generic dictionary}, \emph{CIL metadata}.
    \item Avoid first-person pronouns and conversational language.
\end{itemize}
...
\noindent\textbf{Writing Style Constraint}
\begin{itemize}
    \item Formal, objective, rigorous scientific style; no first-person pronouns.
    \item Precise terminology: \emph{static dispatch}, \emph{boxing overhead}, \emph{data locality}, \emph{reification}, \emph{monomorphization}, \emph{type erasure}, \emph{JIT specialization}, \emph{cache lines}.
    \item Ground all claims in established compiler theory, VM/CLR specifications, and hardware-interaction mechanics; avoid unsupported assertions.
\end{itemize}
...
\noindent\textbf{Writing Style Constraint}
\begin{itemize}
    \item Formal, objective, scientific writing style; no first-person pronouns.
    \item Distinguish clearly between theoretical concepts (parametric polymorphism) and implementation mechanisms (monomorphization, erasure, reification).
    \item Avoid unsupported claims and clearly identify assumptions.
\end{itemize}
...
\begin{enumerate}
    ...
    \item References
\end{enumerate}
Format each code example using the standard \texttt{listings} package.
...
\noindent\textbf{Writing Style Constraint}
\begin{itemize}
    \item Formal, objective, highly scientific academic tone; no first-person pronouns.
    \item Precise terminology: \emph{RTTI}, \emph{type erasure}, \emph{reification}, \emph{type token}, \emph{monomorphization}, \emph{generic metadata}.
    \item Logical progression from theoretical foundations to technical analysis.
\end{itemize}
